...
namespace FamilyBoard.Core.Calendar
{
    ...
    {
        private const string PUBLICHOLIDAYSURL = "https://feiertage-api.de/api/?nur_land=BW&jahr={0}";
        ...
        private static async Task<List<CalendarEntry>> GetHolidaysForYear(DateTime startDate, DateTime endDate, int year)
        {
            ...
            using (var client = new HttpClient())
            {
                ...
                if (holidayResponse.IsSuccessStatusCode)
                {
                    ...
                    var holidays = JsonDocument.Parse(holidaysPayload).RootElement.EnumerateObject();
