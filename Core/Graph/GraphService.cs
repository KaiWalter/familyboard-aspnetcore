...
namespace FamilyBoard.Core.Graph
{
    public class GraphService : IGraphService
    {
        ...
        {
            return GraphServiceClientFactory.GetAuthenticatedGraphClient(
                async () =>
                {
                    string[] scopes = _configuration
                        .GetValue<string>(Constants.GraphScope)
                        ?.Split(' ');
                    ...
                    var token = await app.AcquireTokenSilent(
                            scopes,
                            new MsalAccount
                            {
